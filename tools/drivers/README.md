...
它们**不属于推理引擎本体**（`vllm_shs`），而是跑**同态加密测试 / 社区接力**用的驱动。
...
其中 `chain/sk.bin` 可由驱动自身生成：`T23_PHASE=l0 T23_KEYONLY=1 ./t23lay`（会打印 `KEYONLY: sk.bin saved`）。
...
- **段级 A/B/C/D 的 `max|err| FAIL` 不是判据**：它对照的是「输入未偏离的明文参考」，而真实密文链上层间必然带偏离；归一化会把幅度拉回，最终段带 `[E2E:ref-deviate-ok]` 标记。

## 7. 线程数（实测结论）

**`T23_NT=4` 是当前最优，加线程不提速。** 同机同阶段（`lay0` 的 `[A+B]` 段，基准 225 s）：
...
原因：`[A+B]` 的矩阵乘由 **OpenMP 段**主导（默认已占满 16 逻辑核），而 `vllm_tp` 的 worker 是 **spin-wait** 空转，两者叠加造成超订抢核；同时该阶段受**内存带宽**约束（中间量 ≈112 MB），堆线程无法增加带宽。
...
- **运行 / 打包 / 验证脚本**：`arxiv/repo/tools/relay/`（跑层、打包交接、验包、准确度校验）
- **接力复现指南**：`arxiv/接力复现指南.md`
