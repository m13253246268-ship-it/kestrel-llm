...
而全层档同一模型峰值要 12.7 GB（`--stream-test`）/ 14.2 GB（serve，含 KV）。本节数据为
2026-09-12 单次成矩阵实测：**3 个模型（2B / 8B / 30B-A3B）× {全层, 逐层} ×
{组合⑧基线, 组合①, 组合①+⑤}**，两套口径（`--stream-test` 同口径 A/B、serve 冷/热 + 3 轮追问），
共 14 个 serve 配置点，全程无 OOM、无失败、无回退。
