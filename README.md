...
| 核心优势 | 一句话指标 |
|---|---|
| **逐层推理**（v1 新增） | 权重常驻与模型体积解耦：**8B 权重常驻 4.19 GB → 0.48 GB（8.7×）**，serve 峰值 **0.80 GB**；热态 **TTFT 1.95 s / tpot 377 ms**，5 次采样波动 **±1.8% / ±0.13%** |
...
**典型例子**：RK3588 板只有 16 GB RAM（MemTotal 15.6 GB），Qwen3-30B-A3B-q4 的权重单文件
就有 17.66 GB —— 全层档虽能勉强跑起来，但 serve 峰值已到 14.2 GB，几乎不留 KV 与进程余量；
**逐层档把同一模型的 serve 峰值压到 1.11 GB，余量充足、可稳定服务**。2B 更直观：常驻权重只有
0.38 GB，比全层省 4.6×，代价是热态 tpot 从 113.0 ms 升到 156.7 ms（+38.7%）。

**推荐档是 8B**（2026-09-12 实测，见「性能」§4）：权重常驻 4.19 GB → **0.48 GB（8.7×）**，
serve 峰值 **0.80 GB**；热态 **TTFT 1.95 s、tpot 377 ms**（70 token 端到端 27.9 s），
...
> **30B 是能力上限档，不是推荐档**：16.4 GiB 权重装不进 15.6 GB 页缓存，且默认 thinking 下
> 一轮问答需 3~4 分钟（见「性能」§5）。
...
体积、与层数解耦**——17.7 GB 的 Qwen3-30B-A3B-q4 可以在 16 GB RAM 的 RK3588 上服务，
...
> 14.2 GB 已顶到 16 GB 板（MemTotal 15.6 GB）的上限，逐层档把同一模型压到 1.11 GB，
> 留出全部 KV 与进程余量。
...
|---|---|---|---|---|
| 全层 | 4,189,912 kB | 4,202,988 kB | 67.9 s | **186~189 ms/tok** |
| **逐层** | **480,348 kB（8.7×）** | **487,912 kB** | 63.2 s | 370~378 ms/tok |
...
1. **省内存的收益拿满，代价可控**：权重常驻 4.19 GB → **0.48 GB（8.7×）**，serve 峰值 **0.80 GB**。
2. **热态稳定有物理保障**：权重 6.15 GiB **装得进 15.6 GB 页缓存**，所以热态不必赌运气——
...
| 档 | TTFT | 端到端 | tpot | 峰值 VmHWM |
|---|---|---|---|---|
| 冷（`drop_caches` 后首次） | 211.5 s | 258.8 s | 1,524.5 ms | 933,656 kB |
| **热（紧接着重发同一请求）** | **3.3 s** | **20.8 s** | **564.8 ms** | 933,656 kB |

非流式同请求交叉验证 20.7 s（与流式 20.8 s 一致）。
...
冷态 258.8 s 的代价来自首次从 SD 卡读入全部 16.8 GB 权重，是一次性成本；权重驻留页缓存
后回到秒级——这正是「逐层推理」把 17.66 GB 模型装进 16 GB 板的方式。
...
- **逐层推理可交付化**（`VLLM_VQF_STREAM=1` 分层驻留 + KV v2 惰性分配）——权重常驻 RSS
  实测省 **4.6× / 8.9× / 23.2×**（2B / 8B / 30B-A3B），使 17.7 GB 的 30B-A3B 能在
