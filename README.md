...
| 核心优势 | 一句话指标 |
|---|---|
| **逐层推理**（v1 新增） | 常驻内存与模型体积解耦：**17.66 GB 的 30B-A3B 在 16 GB 板上服务**，逐层档峰值 **1.11 GB**（全层 14.2 GB） |
| **多轮长上下文** | 组合①（L3 驱逐 × 前缀复用）追问轮 prefill **−80% ~ −96%**；30B 再叠 MoE 组合⑤后 t2/t3 仅 **4.7 s / 3.2 s** |
...
> 带宽差 3.8×（见第 4 节），因此本节的 8B/30B 冷读代价按 **SD 卡 62.7 MB/s** 计。
...
#### 4) 30B-A3B 短请求：不只是能跑，而是能用（上文 22 token + 生成 32 token）
...
**结论：30B-A3B 在这块 16 GB 板上的短请求热态，20.8 s 完成 32 token 输出，峰值内存
0.91 GB。** 冷态 258.8 s 的代价来自首次从 SD 卡读入全部 16.8 GB 权重，是一次性成本；
权重驻留页缓存后回到秒级——这正是「逐层推理」把 17.66 GB 模型装进 16 GB 板的方式。
...
- **30B-A3B 在 16 GB 板上的绝对速度很低，这是硬件边界而非实现缺陷**：无优化基线一轮
  ...
  追问降到 3~5 s，是 30B 能被真正用起来的前提。**短请求实测见第 4 节**
  （22 token 上文 → 32 token 输出：热态 20.8 s、峰值内存 0.91 GB）。
